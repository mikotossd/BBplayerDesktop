...
### P16 · 收尾追加之三：歌单行尾「⋯」二级菜单（分享 + 删除歌单）（已完成）

用户给的参考图是**播放列表行尾一颗「⋯」+ 二级菜单**，要求：把左栏歌单行尾那颗
**分享图标**换成「⋯」、把原来的分享逻辑绑到二级菜单上、并**新增「删除歌单」**。

- 行尾换成 `more_vert` 的「⋯」，点开是 `bbComponents.menu`（与曲目行同一套，
  连危险项的红色都一样）：**分享到云端 / 同步到云端** + **删除歌单**
- 「删除歌单」是**新功能**，补了整条链：`db.deletePlaylist` →
  `playlist:delete` IPC → `preload.playlist.delete` → 确认框 → 删 → 退回列表
- 三条设计：只删歌单不动 `tracks`；不依赖 `ON DELETE cascade`（连接级 PRAGMA，
  换个连接就没了）；共享歌单先取消共享，取消失败也照样删本地，但**如实说**
- 顺手：`sharePlaylist` 不再把按钮文字改成「分享中…」（那会把图标按钮变回宽文字
  按钮，又把歌单名挤没）；左栏「⋯」的 `data-action` 从 `more` 改成 `playlist-more`
  （与曲目行撞名，把"每行曲目都有「⋯」"那条断言数错了）

**又一条差点写成恒真的断言**：我原来查"行上还有没有 `[data-action="share"]`"，
而那个属性值改完就不存在了 → 永远为真。换成量**行尾按钮数 === 1**。

反馈：`verify:desktop:ui` **231 / 0**（+7 条），其余套件与上一轮相同（全 0 失败），
`tour` 浅 / 深各 37 张 0 问题。
