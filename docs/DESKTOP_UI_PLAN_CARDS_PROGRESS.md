...
### 下一件：阶段 6 · 歌单详情页头 + 紧凑歌曲行（song-row）

**基线**：`verify:desktop:ui` = **212 项全通过**。

**具体改动**

1. 歌单详情页头按草图：封面 + 标题 + 「N 首 · 总时长」+ 播放全部 / 多选，
   用 `.page-head` 那一套（标题只有一处）。
2. 曲目行从「表格行」改成**紧凑行卡**（`.song-row`）：
   序号 / 封面 / 标题+歌手 / 时长 / 「⋮」；保留 `[data-testid="track-table"]`
   与 `tbody tr` 结构 —— `history.js` 的 `getRowCount()` 与
   `ui-probe-driver` 的多条断言**硬依赖**它们（施工单里标过）。
3. ⚠️ 多选态的复选框、拖拽把手、行内「⋮」都要继续可达（探针有断言）。
4. 新增断言：行高 ≤ 56px、封面是圆角方形、悬停才露出「⋮」、
   选中行的底色与左栏胶囊是同一套令牌。
