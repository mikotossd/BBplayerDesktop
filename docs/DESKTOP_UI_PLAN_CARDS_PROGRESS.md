...
**基线**：`verify:desktop:ui` = **212 项全通过**。

**具体改动**

1. 收藏夹列表从"一行一个（`.list-row`）"改成**卡片**（封面 + 名称 + 视频数），
   与「音乐库 › 播放列表」同一张 `mediaCard`。
2. **用户确认过：收藏夹卡只显示 N 个视频**（不放预览列表）。
   现在展开预览会内嵌一张曲目表（`renderTrackTable(items, { into })`），
   按这条要求应当**删掉那个内嵌表格渲染器**：
   - `library.js` 的 `renderTrackTable` 的 `into` 分支；
   - `favorites.js` 里调用它的那段（`:274`）；
   - `history.js` 的 `getRowCount()` 依赖 `[data-testid="history-table"] tbody tr`
     —— 那是**历史表**，与收藏夹预览无关，**不要一起删**。
3. ⚠️ 收藏夹的「全量渲染」断言、`favorite-status` 文案、刷新按钮、
   「换个 UID…」工具条都要保留（探针有 6 条断言）。
4. ⚠️ 封面仍是**视频封面**；没有封面时走首字色块（不要新造一套占位）。
