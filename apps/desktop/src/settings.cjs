...
const DEFAULT_SETTINGS = {
	 ...
	 * ⚠️ 默认**浅色**（而不是跟随系统）。
	 *
	 * 卡片化改造时用户明确要求："我希望现在主题色默认是白色"。
	 * 跟随系统在深色系统上会立刻变成深色，与"默认白色"矛盾 ——
	 * 所以默认值就是浅色，用户仍然可以在「设置 › 主题」里改成跟随系统或深色。
	 */
	theme: 'light',
	 ...
	 * ⚠️ 默认改成**自定义 + 白色**（卡片化改造）。
	 *
	 * 用户要求"主题色默认是白色，而不是现在的粉色"。跟随系统强调色在
	 * Windows 上常常拿到品牌紫/粉，屏幕上到处是彩色；而白色种子经
	 * `deriveSchemeFromAccent` 生成的是**纯灰阶**（配合 accent.ts 里
	 * 新加的"无彩色种子"分支），于是**屏幕上的彩色只来自视频封面**。
	 */
	accentMode: 'custom',
