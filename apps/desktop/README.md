...
- **队列只有一份 DOM**（`#queue-list`），常驻在**播放列表浮层**里
  （卡片化收尾删掉了右栏）。渲染两份会让 `data-queue-index` 重复。
...
- **行右侧是「⋮」而不是并排图标**（安卓端如此，桌面端照做）。关闭时机有**三条**：
  ...
  ⚠️ 左栏**歌单行**也是这个形状（卡片化收尾）：行尾一颗 `more_vert` 的「⋯」，
  菜单里是「分享到云端 / 同步到云端」+「删除歌单」。分享原来直接摆在行上 ——
  一颗图标只能表达一个动作，**"删除歌单"因此没有地方放**；而且分享/同步是低频
  且不可逆的动作，不该每行一颗。
  定位属性别撞名：歌单行是 `data-action="playlist-more"`，曲目行是
  `data-action="more"`（撞过一次，"每行曲目都有「⋯」"那条按
  `[data-action="more"]` 计数的断言把左栏两颗也数了进去）。
- **删除歌单只删歌单，不动 `tracks` 表**：同一首歌可能还在别的歌单与收藏夹里，
  "删歌单"不该顺手把曲目从库里抹掉。且**不依赖 `ON DELETE cascade`**（级联要
  `PRAGMA foreign_keys = ON`，而那是**连接级**的）—— 显式删关联行。
  共享歌单先 `share.unshare`，取消失败也照样删本地，但状态栏要如实说
  "远端可能还留着"。
...
- **控件的外观必须与它是否可交互一致**：禁用的滑杆如果还画着主色轨道，
  ...
  DOM 正常、尺寸正常，**只有人眼看得出**。
- **正在播放页是两栏**（卡片化阶段 4，按草图「放弃原有的三联卡片式」）：
  左卡 = 大封面 + 曲名/歌手/进度/主控，右卡 = **歌词**，各栏各自滚动。
- **播放列表只有一种形态：从下方呼出的浮层**（卡片化收尾）。它在 DOM 里常驻，
  触发器是播放条上的队列按钮、播放详情页的队列按钮与 `Ctrl+Q`；
  呼出**不换页**（就在当前页弹出来）。
  ⚠️ 歌词面板是**唯一一份**，而且**常驻**在播放详情页的右卡里。那个 `.panel`
  壳**没有 `id`**（只有 `data-panel`）—— 按 id 找会静默拿到 null，
  于是右卡永远空着，**而所有断言照过**（第一版就是这样，靠新断言 + 截图才发现）。
- **「呼出 / 收起播放列表」四处共用一个实现**（`toggleQueuePopover()`）：播放条上的
  按钮（按参考图放在「下一首」右边）、播放详情页的队列按钮、`Ctrl+Q`、
  以及顶部菜单那一项。
- **队列行 = 序号 + 曲绘封面 + 标题/歌手 + 时长**（与其它列表行同一套约定）。
- **左栏宽度可拖拽**（`renderer/splitter.js`）：`.app` 第一列走 `--sidebar-width`，
  于是"拖到某个宽度"与"收起（0）"**互不覆盖**（两个状态都去写整条
...
- **主页的组成与顺序按原型**（`prototype/pages/home.html`，卡片化收尾定稿）：
  **最近播放 | 快捷入口**（顶部左右分栏，中间一条竖线）→ **听歌频率**（热力图）
  → **最近更新**（歌单卡网格）→ **播放历史**（页签 + 汇总 + 播放全部在标题行里，
  下面是歌曲行）。四块**全部在内容卡内部** —— 曾经把前两块放在内容卡
  **下面**一条独立的下沿里（`#home-strip`），主页因此看起来是"一张卡片 +
  一个孤零零的盒子"，用户反馈"太混乱"，于是整条下沿删掉了。
  ⚠️「最近更新」**不是**「近期歌单」：安卓端那条按 `updatedAt` 排序，而
  `playlists.updatedAt` 只在歌单被**修改**时才动（播放写 `play_history`，
  不碰它）。所以右侧那句说明如实写「按歌单最近一次修改排序（不是最近听过）」。
- **播放历史是歌曲行，不是表格**（卡片化收尾）。「播放次数 / 上次听到」这两个
  只在某些页签下存在的列改成**行右侧那一格**（`data-kind` 标着是哪种），
  相对时间并进歌手那一格 —— 表格那套"页签切换就换一组列"在主页里太吵。
- **区块标题行**（`.home-section__head`）= 标题 + 推开 + 右侧说明。
  说明**不另起一行**：四块各占一行灰字就是四行废话（原型也不是那么画的）。
...
（渲染进程之间不能直接通信）。用 Ctrl+Alt+L 或播放详情页歌词卡底部的
「独立窗口」开关。

**它也是主窗口歌词渲染问题的可用绕过路径**：主窗口歌词面板那个已知问题
