...
/**
 * 图标字号（Material Symbols 的字面尺寸）。
 *
 * ⚠️ 与字阶**刻意分开**：M3 的字阶描述文字（body/title/label），而按钮里那些
 * 字形图标的字号表达的是图标大小。混在一起之后"调正文会顺带改掉图标"。
 *
 * 这两个值原来只写在 `apps/desktop/src/renderer/style.css` 的兜底块里 ——
 * 于是它们是**第二份真相**：改令牌包不会影响它们，改它们也不会被令牌包发现。
 * 现在由令牌包统一给出，`theme.cjs` 下发，style.css 只保留同值兜底。
 */
